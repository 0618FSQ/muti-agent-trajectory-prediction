...
class FutureCrossEncoder(nn.Module):
    def __init__(self, hidden_size, sub_layers):
        super().__init__()
        self.future_vector_net = VectorNet(
    ...
    def forward(self,
        ...
        attention = torch.sigmoid(target_agent_history_feature * future_traj_feature)
        future_traj_feature = target_agent_history_feature * (1 - attention) + future_traj_feature * attention
...
class SceneModel(nn.Module):
    def __init__(
        ...
        self.future_trajectory_vectornet = VectorNet(
            in_channels=hidden_size,
            hidden_size=hidden_size,
            sub_layers=sub_layers
        )
